<template>
  <div>
    ...
    <section class="container">
      ...
      <section class="mid">
        <ItemPage>
          <AdelaidePage v-if="udata.Adelaide[6]!=0&&tdata.Adelaide[6]!=0" :tdata="tdata" :udata="udata"></AdelaidePage>
        </ItemPage>
        <ItemPage>
          <MelbournePage v-if="udata.Melbourne[6]!=0&&tdata.Melbourne[6]!=0" :tdata="tdata" :udata="udata"></MelbournePage>
        </ItemPage>
      </section>
      <section class="right">
        <ItemPage>
          <BrisbanePage v-if="udata.Brisbane[6]!=0&&tdata.Brisbane[6]!=0" :tdata="tdata" :udata="udata"></BrisbanePage>
        </ItemPage>
        <ItemPage>
    ...
    const getUnemployment = (listData) =>{
      ...
      for(let i =0;i<listData.length;++i){
        if(cacheData[i].city=='Adelaide'){
          udata.Adelaide[Number(cacheData[i].month)-2]=cacheData[i].unemployment/100
        }else if(cacheData[i].city=='Brisbane'){
          udata.Brisbane[Number(cacheData[i].month)-2]=cacheData[i].unemployment/100
        }else if(cacheData[i].city=='Darwin'){
          udata.Darwin[Number(cacheData[i].month)-2]=cacheData[i].unemployment/100
        }else if(cacheData[i].city=='Melbourne'){
          udata.Melbourne[Number(cacheData[i].month)-2]=cacheData[i].unemployment/100
        }else if(cacheData[i].city=='Hobart'){
          udata.Hobart[Number(cacheData[i].month)-2]=cacheData[i].unemployment/100
        }else if(cacheData[i].city=='Perth'){
          udata.Perth[Number(cacheData[i].month)-2]=cacheData[i].unemployment/100
        }else{
          udata.Sydney[Number(cacheData[i].month)-2]=cacheData[i].unemployment/100
        }
      }
      console.log(udata)
    };
