...
export default {
  ...
  setup() {
    ...
    const getTweets = listData => {
      ...
      for (let i = 0; i < listData.length; ++i) {
        if (cacheData[i].key.search("Adelaide")!=-1) {
          tdata.Adelaide[Number(cacheData[i].month) - 2] =
            cacheData[i].unemployment / 100;
        } else if (cacheData[i].key.search("Brisbane")!=-1) {
          tdata.Brisbane[Number(cacheData[i].month) - 2] =
            cacheData[i].unemployment / 100;
        } else if (cacheData[i].key.search("Darwin")!=-1) {
          tdata.Darwin[Number(cacheData[i].month) - 2] =
            cacheData[i].unemployment / 100;
        } else if (cacheData[i].key.search("Melbourne")!=-1) {
          tdata.Melbourne[Number(cacheData[i].month) - 2] =
            cacheData[i].unemployment / 100;
        } else if (cacheData[i].key.search("Hobart")!=-1) {
          tdata.Hobart[Number(cacheData[i].month) - 2] =
            cacheData[i].unemployment / 100;
        } else if (cacheData[i].key.search("Perth")!=-1) {
          tdata.Perth[Number(cacheData[i].month) - 2] =
            cacheData[i].unemployment / 100;
        } else {
          tdata.Sydney[Number(cacheData[i].month) - 2] =
            cacheData[i].unemployment / 100;
        }
      }
    };
    ...
    onMounted(() => {
      getState().then(() => {
        console.log(NegData.data.data[0].key.search('Adelaide'));
        console.log(AllData.data.data[0].key);
      });
    });
    const goTo1 = () => {
      router.push({
